...
router.post('/register-face', authenticateAdmin, async (req, res) => {
  ...
  try {
    ...
    await sequelize.query(
      'INSERT INTO users (employee_id, username, name, department, face_embedding, role) VALUES (:employee_id, :username, :name, :department, :faceEmbedding, \'employee\')',
      {
        replacements: { employee_id, username: employee_id, name, department, faceEmbedding },
...
router.put('/employees/:id', authenticateAdmin, async (req, res) => {
  ...
  const { username, name, department, faceEmbedding } = req.body;

  if (!username || !name || !department || !faceEmbedding) {
    return res.status(400).json({ message: 'Missing required fields' });
  }

  try {
    await sequelize.query(
      'UPDATE users SET username = :username, name = :name, department = :department, face_embedding = :faceEmbedding WHERE id = :id',
      {
        replacements: { username, name, department, faceEmbedding, id: employeeId },
